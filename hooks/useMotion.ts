...
export function useMotion() {
  useEffect(() => {
    ...
    const fx = () => {
      ...
      if (how && parseFloat((how as HTMLElement & { dataset: DOMStringMap }).dataset.maxx ?? '0') > 0) {
      ...
      if (stEl) {
        ...
        if (mode === 'skew' && onScreen) {
          const now = box.top
          const vel = stPrev == null ? 0 : stPrev - now
          stPrev = now
          const sk = Math.max(-9, Math.min(9, vel * 0.32))
          stEl.style.transform = `skewX(${sk.toFixed(2)}deg)`
        } else if (onScreen) {
